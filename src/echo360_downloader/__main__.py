...
def _prompt_lecture_selection(lectures: list[dict], course_name: str) -> list[int]:
    ...
    try:
        ...
        info("Aborted.")
        sys.exit(0)
    return _parse_selection(raw, len(lectures))
...
async def _cmd_download(
    ...
    async with async_playwright() as p:
        ...
        if target is None:
            indices = _prompt_lecture_selection(lectures, course_name)
        else:
